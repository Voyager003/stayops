...
- **채널 등록·관리**: FineStay(자사 직영) + OTA(Agoda, Airbnb 등) 채널을 등록하고 수수료율·상태를 관리할 수 있어야 한다
- **FineStay 자동 생성**: 숙소 등록 시 FineStay 채널이 자동 생성되어야 한다 (수수료 0%, DIRECT 타입)
- **재고 동기화**: 예약 생성/취소 시 모든 활성 OTA 채널에 재고 변경을 자동 전파해야 한다
- **외부 예약 수신**: OTA로부터 웹훅을 통해 예약 생성/취소 요청을 수신하고 처리할 수 있어야 한다
- **동기화 대시보드**: 채널별 동기화 상태(성공/실패/대기)를 모니터링할 수 있어야 한다

## 기술적 도전 과제

| 과제 | 설명 | 해결 전략 |
|------|------|----------|
| 메시지 브로커 없는 비동기 동기화 | Kafka/RabbitMQ 없이 신뢰성 있는 채널 동기화 필요 | Outbox 패턴 — SyncTask를 비즈니스 트랜잭션과 동일 트랜잭션에 저장, 스케줄러가 폴링 |
| 동기화 실패 복구 | OTA API 호출 실패 시 데이터 유실 방지 | 재시도(maxRetries=3) + FAILED 상태로 전환, 대시보드에서 수동 재시도 가능 |
| 웹훅 보안 | 외부에서 위조된 웹훅 요청 방지 | HMAC 서명 검증 — ChannelConfig.webhookUrl의 secret으로 페이로드 서명 확인 |
| 실제 OTA 없이 테스트 | 개발 환경에서 Agoda/Airbnb API를 호출할 수 없음 | `VirtualChannelSyncAdapter` — ~80% 성공, ~20% 실패를 시뮬레이션 |
