...
| 날짜별 재고 차감/복원 | Inventory | `RoomInventory.reserve()`, `RoomInventory.release()` | 도메인 기능은 적합하나 여러 Application이 `RoomInventoryApplication`을 직접 호출한다. 모듈 분리 전 Port가 필요하다. |
...
| `RoomInventory` | private constructor + factory/reconstitute, 계산 속성/재고 변경 메서드 제공 | 재고 변경 기능은 Aggregate에 있으나 Application 간 참조가 경계 위반을 만든다. |
