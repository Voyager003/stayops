...
- `AvailabilitySyncPort`: Inventory가 Channel Application을 직접 호출하지 않고 OTA 가용 재고 동기화 요청 계약에 의존한다. 2026-04-12에 적용했다.
- `RoomInventorySyncPort`: Room이 Inventory Application 구현체를 직접 호출하지 않고 객실 생성 후 재고 동기화 계약에 의존한다. 2026-04-12에 적용했다.
- 대규모 이벤트 전환은 원자성 요구사항을 먼저 검토한 뒤 진행한다.

2026-04-12에 해소한 직접 참조:
