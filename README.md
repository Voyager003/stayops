...
- C. [프로젝트 달성 목표](#c-프로젝트-달성-목표)
  ...
  - b. [가용성을 고려한 시스템 설계](#b-가용성을-고려한-시스템-설계)
  - c. [도메인 설계에 대한 고민](#c-도메인-설계에-대한-고민)
...
이 프로젝트에서 `Mock OTA`의 역할은 PMS가 외부 판매 채널과 연동될 때 필요한 핵심 흐름을 검증하기 위한 시뮬레이터입니다. PMS에서 OTA로 객실 재고를 동기화하는 흐름, OTA에서 예약·취소가 발생했을 때 Webhook을 PMS가 수신해 내부 예약과 재고에 반영하는 흐름을 구현했습니다.
...
`Production`은 서버 Scale-out 및 DB Replica set(P-S-S)뿐만 아니라 Logging/Metric을 지원하는 가용성을 도모한 아키텍처입니다.
...
이 프로젝트에서는 실제 서비스를 운영하기 위한 상황을 예상하여 서비스 가용성과 장애 대응 가능성을 검증하는 것을 주요 목표로 삼았습니다.

또한 숙박 도메인에서 발생하는 숙소 예약 및 결제, 재고 동기화, 외부 API 연동(PG 호출)에서 생길 수 있는 문제를 제시하고 이를 해결합니다.
...
이런 개념들을 하나의 서비스 로직에서 상태값으로만 처리하면, 예약이 언제 재고를 점유하는지, 결제 실패가 예약에 어떤 영향을 주는지, 외부 채널 예약을 내부 예약과 같은 방식으로 다뤄도 되는지 같은 규칙이 흐려질 수 있다고 판단했습니다.
