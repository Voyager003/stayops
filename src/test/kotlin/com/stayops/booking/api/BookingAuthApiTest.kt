...
class BookingAuthApiTest {
    ...
    private val customerAuthService = mockk<CustomerAuthService>()
    private val mockMvc: MockMvc = MockMvcBuilders
        .standaloneSetup(BookingAuthApi(customerAuthService))
    ...
    inner class `고객_회원가입` {
        ...
        fun `유효한 요청이면 201을 반환한다`() {
            every { customerAuthService.signup("customer@test.com", "password123", "김고객") } returns sampleCustomer()
        ...
        fun `중복 이메일이면 409를 반환한다`() {
            every { customerAuthService.signup(any(), any(), any()) } throws
    ...
    inner class `고객_로그인` {
        ...
        fun `CUSTOMER 계정이면 200을 반환한다`() {
            every { customerAuthService.login("customer@test.com", "password123") } returns sampleCustomer()
        ...
        fun `스태프 계정이면 400을 반환한다`() {
            every { customerAuthService.login(any(), any()) } throws
