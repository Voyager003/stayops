...
class CustomerAuthApiTest {

    private val customerAuthApplication = mockk<CustomerAuthApplication>()
    private val mockMvc: MockMvc = MockMvcBuilders
        .standaloneSetup(CustomerAuthApi(customerAuthApplication))
    ...
    inner class `고객_회원가입` {
        ...
        fun `유효한 요청이면 201을 반환한다`() {
            ...
            mockMvc.post("/api/v1/customer/auth/signup") {
    ...
    inner class `고객_로그인` {
        ...
        fun `CUSTOMER 계정이면 200을 반환한다`() {
            ...
            mockMvc.post("/api/v1/customer/auth/login") {
                contentType = MediaType.APPLICATION_JSON
    ...
    inner class `고객_로그아웃` {
        ...
        fun `로그아웃하면 204를 반환한다`() {
            mockMvc.post("/api/v1/customer/auth/logout")
                .andExpect {
                    status { isNoContent() }
                }
        }
